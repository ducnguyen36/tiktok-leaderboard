...
// As long as this server runs, the data persists.
let dailyState = {
...
// 1. Process Monthly Data (Standard filtering & mapping)
const processMonthlyData = (rawData) => {
    if (!rawData?.data?.anchorAnalysisDataInfos) return [];
    
    const filteredData = rawData.data.anchorAnalysisDataInfos.filter(item => {
        const creatorId = item.anchorBaseInfo?.user_base_info?.CreatorID;
        return ALLOWED_CREATOR_IDS.includes(creatorId);
    });
...
// 2. Process Daily Data (Accumulation Logic)
const processDailyData = (rawData) => {
    // Check for Day Reset
    const today = new Date().toLocaleDateString();
    if (today !== dailyState.lastDate) {
        console.log(`[${new Date().toLocaleTimeString()}] 🌙 New day detected! Resetting daily history.`);
        dailyState.history = {};
        dailyState.lastDate = today;
    }

    if (!rawData?.data?.LiveAnchorInfos || !rawData?.data?.HostBaseInfoMap) {
        // If API returns empty/error but we have history, return current history
    ...
    LiveAnchorInfos.forEach(info => {
        ...
        if (!ALLOWED_CREATOR_IDS.includes(creatorId)) return;

        // Initialize Creator in History if new
        ...
        // Update Profile (in case they changed avatar/name)
        ...
        // Update Room Score (Accumulation Logic: Store max seen for this room)
        // We assume the API returns the cumulative total for the current room session.
...
app.get('/api/leaderboard', async (req, res) => {
    ...
    console.log(`[${new Date().toLocaleTimeString()}] Processing ${type} request...`);
    ...
    if (type === 'daily') {
        ...
        queryString = "startTime=1764547200&endTime=1765238399&sortTagEnum=1&descOrAsc=desc&offset=0&limit=20&FactionID=100579&compareType=1&timeType=4&msToken=uGVeIcAtwEcW9ZaDBHDpFMzsOJUfNTQ3U5YnMEUbxId-8wjhRaVnGrxsUCXWGaK6cOhBa2yFWPEMKG0GXycFvnD37JBS8pHgSHmNeFlRXurC1gWoU2RHjPOMBU85Eb8=&X-Bogus=DFSzswVY92DdUAOFCYH-GvpJlh87&X-Gnarly=MkWQjtqhymNiY6mUPL8ODEM3qOCWy9Rzz4x4-B2xtdwSrhJNuNvl4DwofXiPFmuNLOtFw/RxWH3KiZUPnyt5AJnD4xnFQsdhTQXW/XSrwFFP-WJRadP5LT5sU84K7Sqb3vzWekhrqByXp5TanmehI5ox8DusLhzQ1XP8PA-r4IAbV4IZHFL7PrUJNQqwAF9xZw1DjfBwlFmD-v41WBiAW0kZ4jda/LcDphSxz7arJXb6oqLvULNURlMdzn-BR46F1fvqqRq34NC5ILmsgAgTeal79zToZ7U3mcOXdKGF2yvu";
        headers = {
            'accept': 'application/json, text/plain, */*',
    ...
    try {
        ...
        if (type === 'daily') {
            processedData = processDailyData(json);
        } else {
            processedData = processMonthlyData(json);
        }
