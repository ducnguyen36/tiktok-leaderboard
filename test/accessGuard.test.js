...
function fixture() {
  ...
  const context = { document, localStorage, AbortController, setTimeout, clearTimeout,
    ...
    async fetch(url) {
      if (url === '/auth/status') {
        if (statusFailure === 'network') throw Error('container restarting');
        if (statusFailure === 'server') return { ok: false, status: 502, json: async () => ({}) };
        return { ok: true, json: async () => ({ authorized: allowed }) };
      }
      return { ok: true, json: () => new Promise(resolve => { lateResolve = resolve; }) };
    }
  };
  context.window = context; vm.runInNewContext(source, context);
  return { context, intervals, events, deny() { allowed = false; }, failStatus(mode) { statusFailure = mode; }, navigation: () => navigation, release() { lateResolve({ names: 'late secret' }); } };
}
...
for (const failure of ['network', 'server']) test(`temporary ${failure} failure during deploy preserves authorization and saved settings`, async () => {
  const f = fixture(); await flush(); f.failStatus(failure); await f.intervals[0]();
  assert.equal(f.context.document.body.textContent, 'Private rankings');
  assert.equal(f.context.document.documentElement.style.visibility, undefined);
  assert.equal(f.navigation(), undefined);
  assert.equal(f.context.localStorage.helios_leaderboard_v2_current, 'private names');
  assert.equal(f.context.localStorage.helios_leaderboard_v2_default, 'private names');
});
